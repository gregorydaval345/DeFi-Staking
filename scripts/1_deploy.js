...
async function main() {
  ...
  transaction = await staking.connect(signer2).stakeEther(10, data);
  ...
  await staking.connect(signer1).changeUnlockDateForPosition(3, newUnlockDate);
  ...
  block = await provider.getBlock(receipt.blockNumber);
  newUnlockDate = block.timestamp - 60 * 60 * 24 * 100;
  await staking.connect(signer1).changeUnlockDateForPosition(4, newUnlockDate);
}
